...
impl TokenScore {
    ...
    pub fn calculate(
        ...
        let volume_ratio = if volume_thresh > 0.0 {
            total_volume / volume_thresh
        } else {
            1.0
        };
        ...
        let buyers_ratio = if buyers_thresh > 0 {
            buyers as f64 / buyers_thresh as f64
        } else {
            1.0
        };
        ...
        let late_velocity_ratio = if velocity_thresh > 0.0 {
            late_volume / velocity_thresh
        } else {
            1.0
        };
        ...
        let early_accel_ratio = if volume_thresh > 0.0 {
            early_volume / volume_thresh
        } else {
            0.0
        };
...
impl TokenFilter for ScoreFilter {
    ...
    fn evaluate(&self, token: &TokenData, activity: &TokenActivity, _market_ctx: &MarketContext) -> FilterResult {
        ...
        if self.config.enable_early_acceleration_bias && score.breakdown.momentum_score < 3.0 {
            let early_momentum_strong =
                score.breakdown.early_acceleration_score >= 20.0 && activity.unique_buyers > 5;
